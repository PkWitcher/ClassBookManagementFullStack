...
import { getClasses, getSessions, createSession, bookSession } from '../utils/api';
            ...
            {user?.role === 'Admin' && (
              <div style={{ marginBottom: '2rem' }}>
                <div style={{ 
                  ...
                  <h2 style={{ color: '#2c3e50', margin: 0 }}>Create New Session</h2>
                  <button
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                              ...
                              <div>
                                ...
                                <span style={{
                                  ...
                                  borderRadius: '4px',
                                  fontSize: '0.8rem',
                                  background: session.bookedSeats >= session.capacity ? '#e74c3c' : '#27ae60',
                                  color: '#fff',
                                }}>
                                  {session.bookedSeats >= session.capacity ? 'Full' : 'Available'}
                                </span>
                              </div>
                            </div>
                          </div>
                          ...
                          {user?.role !== 'Admin' && (
                            <button
                              ...
                              disabled={session.bookedSeats >= session.capacity || isBooking}
                              style={{
                                padding: '0.5rem 1rem',
                                borderRadius: '6px',
                                border: 'none',
                                cursor: session.bookedSeats >= session.capacity || isBooking ? 'not-allowed' : 'pointer',
                                background: session.bookedSeats >= session.capacity || isBooking ? '#ccc' : '#e67e22',
                                color: '#fff',
                                fontWeight: 500,
                                transition: 'all 0.2s ease-in-out',
                                whiteSpace: 'nowrap',
                              ...
                              onMouseEnter={(e) => {
                                if (!(session.bookedSeats >= session.capacity || isBooking)) {
                            ...
                            >
                              {session.bookedSeats >= session.capacity ? 'Full' : isBooking ? 'Booking...' : 'Book'}
                            </button>
                          )}
                        </div>
                      </div>
